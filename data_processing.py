...
for locationpath in data_path.glob('*'):
    for orientationpath in locationpath.glob('*'):
        for i, fn in enumerate(tqdm(sorted(list(orientationpath.glob('*.mat'))))):
            ...
            def process(data):
                ...
                if 'Cor' in orientation and 'Heidel' not in location:
                    data = np.flip(data, axis=-2)
                if 'Min' in location and data.ndim == 4:
            ...
            with h5py.File(fn, 'r') as f:
                ...
                b1 = process(
                    np.array(f['CombB1RFromAFI']['real'] + 1j * np.array(f['CombB1RFromAFI']['imag'])).astype(
                        np.complex64
                    )
                )
